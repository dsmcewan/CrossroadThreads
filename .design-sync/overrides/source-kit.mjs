...
// `export default` components as NAMED exports (this DS ships default exports
// only; plain `export *` drops them, so nothing lands on window.<GLOBAL>), and
// must shim `process` for browser previews (app code reads process.env.*).
//
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    // FORK: this DS uses `export default function <Name>` throughout; a plain
    // `export * from` drops defaults, so nothing reaches window.<GLOBAL>.
    // Recover each file's default-export name and emit a NAMED re-export
    // alongside the `export *` (which still captures any named exports).
    // Also shim `process` first — app code (e.g. asset()) reads process.env.*,
    // which is undefined in the browser IIFE and throws at render time.
    const nameProject = new Project({
      skipAddingFilesFromTsConfig: true,
      compilerOptions: { jsx: ts.JsxEmit.Preserve, allowJs: true, skipLibCheck: true },
    });
    const entryLines = ['globalThis.process = globalThis.process || { env: {} };'];
    for (const p of comps) {
      const sf = nameProject.addSourceFileAtPathIfExists(p);
      let defName = null;
      if (sf) {
        for (const [name, decls] of sf.getExportedDeclarations()) {
          if (name !== 'default') continue;
          defName = decls.map((d) => d.getName?.()).find((n) => n && n !== 'default') || null;
        }
      }
      if (defName && /^[A-Z][A-Za-z0-9]*$/.test(defName)) {
        entryLines.push(`export { default as ${defName} } from ${JSON.stringify(p)};`);
      }
